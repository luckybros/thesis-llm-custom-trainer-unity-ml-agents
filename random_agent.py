...
def create_and_run_environment(base_seed, current_writer, run_count):
    ...
    try:
        while global_step_count < TOTAL_STEP_BUDGET:
            ...
            #if oracle_channel.bug_detected in ['error', 'exception', 'hang']:
            #    print(f"Bug critico intercettato ({oracle_channel.bug_detected}). Chiusura ambiente...")
            #    break
            #elif oracle_channel.bug_detected != 'None':
            #    print(f"Bug non critico intercettato ({oracle_channel.bug_detected}). Reset dell'episodio...")
            #    oracle_channel.bug_detected = 'None'
            #    env.reset()
            #    continue
...
try:
    for current_seed in range(6, NUM_SEEDS + 1):
        global_step_count = 0
        run_count = 0
        accumulated_stats = {}
